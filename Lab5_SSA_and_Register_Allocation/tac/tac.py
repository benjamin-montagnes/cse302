...
class Instr:
    ...
    def __str__(self):
        ...
        if self.opcode == 'label':
            ...
            result.write(', '.join(f'{lab}:{tmp}' for lab, tmp in self.arg1))
            result.write(');')
        else:
            result.write('  ')
            if self.dest != None:
...
def __create_parser():
    ...
    def p_phidef(p):
        ...
        p[0] = Instr(p[1], 'phi', tuple(p[5]), None)
        ...
        if instr.opcode == 'nop':
            ...
            for lab, tmp in instr.arg1:
                ...
                raise RuntimeError(f'cannot resolve phi: '
                                   ...
                                   f'can only handle [{",".join(lab for lab, _ in instr.arg1)}]')
